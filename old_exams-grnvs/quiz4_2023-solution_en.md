...
- Only those results will be graded where the solution path is recognizable. Textual tasks must generally be justified unless explicitly stated otherwise in the respective sub-tasks.  
...
Marks can be crossed out by filling them in completely.  
Crossed-out answers can be marked next to them to indicate they should be counted.  
...
d) *You observe a TCP segment. The first 8 bytes of the TCP header are shown in Figure 1.1. What service is the sender likely trying to use?  
```
0x0000 16 00 00 19  
0x0004 73 63 b0 f0  
```
...
- Flow control adjusts the reception window.  
- Congestion control tries to avoid overload in the network.  
- Flow control tries to avoid overload in the network.  
...
- Simpler connection tracking on the server.  
- TCP can terminate the connection, so the server is informed.  
